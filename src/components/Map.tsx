...
const Map: React.FC<MapProps> = ({
  ...
  useEffect(() => {
    ...
    if (pickupCoords) {
      ...
      if (pickupMarker.current) {
        ...
        pickupMarker.current = L.marker(latLng, { icon: createPickupIcon() })
          .addTo(map.current);
      }
    }
  ...
  useEffect(() => {
    ...
    if (dropoffCoords) {
      ...
      if (dropoffMarker.current) {
        ...
        dropoffMarker.current = L.marker(latLng, { icon: createDropoffIcon() })
          .addTo(map.current);
      }
  ...
  useEffect(() => {
    ...
    if (driverCoords) {
      ...
      if (driverMarker.current) {
        ...
        driverMarker.current = L.marker(latLng, { icon: createDriverIcon() })
          .addTo(map.current);
      }
    }
  ...
  useEffect(() => {
    if (!map.current || !isMapReady || !showRoute || !pickupCoords || !dropoffCoords) return;
    ...
    const drawRoute = async () => {
      try {
        ...
        if (route && map.current) {
          ...
          const coordinates = route.coordinates.map((coord: [number, number]) => [coord[1], coord[0]]);
