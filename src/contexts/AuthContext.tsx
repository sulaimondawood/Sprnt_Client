...
import { UserAccount, RiderProfile, DriverProfile } from '@/types';
import { 
  mockRiderUser, 
  mockDriverUser, 
  mockRiderProfile, 
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [simulateIncompleteProfile, setSimulateIncompleteProfile] = useState(false);
  ...
  const login = async (email: string, password: string, role: 'RIDER' | 'DRIVER') => {
    ...
    if (role === 'RIDER') {
      ...
      setRiderProfile(simulateIncompleteProfile ? mockIncompleteRiderProfile : mockRiderProfile);
      setDriverProfile(null);
    } else {
      setUser(mockDriverUser);
      setDriverProfile(simulateIncompleteProfile ? mockIncompleteDriverProfile : mockDriverProfile);
  ...
  const updateDriverProfile = (profile: Partial<DriverProfile>) => {
    if (driverProfile) {
      setDriverProfile({ ...driverProfile, ...profile, isProfileComplete: true });
    }
  };
